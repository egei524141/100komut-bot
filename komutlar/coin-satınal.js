    ...
    console.log(reqconf.ownerID)
        ...
        try{
            ...
            setTimeout(() => {
                ...
                const findpls = dbfetch.find(el => el.esya.toLowerCase() == esya.toLowerCase())
                const getindex = dbfetch.indexOf(findpls)
                delete dbfetch[getindex]
                const res = dbfetch.filter((el) => el)    
                db.set("marketall", res)              
            }, 1000);
        }catch(e) {
